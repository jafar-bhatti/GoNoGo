...
% send params to arduino %% Here are the trial parameters
fprintf(p,'%f %f %f %f %d ',[params.holdD params.respD ...
    params.rewardDuration params.timeoutD params.debounceTime]);

%%% What is unclear to me is where the trial structure is determined. It
%%% seems like it is written into the arduino
...
while cnt < 2000
    ...
    if contains(out,'TRIAL')
        ...
        % determine offset
        %tt(cnt,2) = randi(size(stim,2),1); %%This is the part I need to figure out!
                        
        % queue stimulus 
        queueOutput(s,low_sound,params.device);
